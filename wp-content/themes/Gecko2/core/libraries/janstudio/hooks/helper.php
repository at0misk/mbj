...
if ( ! function_exists( 'jas_gecko_header' ) ) {
	function jas_gecko_header() {
		...
		ob_start();
		if ( $layout == NULL ) {
			get_template_part( 'views/header/layout', 5 );
		} else {
...
if ( ! function_exists( 'jas_gecko_head_page' ) ) {
	function jas_gecko_head_page() {
		...
		if ( is_home() ) {
			...
			$atts = 'style="background: url(' . esc_url( $blog_head_bg['image'] ) . ') ' . esc_attr( $blog_head_bg['repeat'] ) . ' ' . esc_attr( $blog_head_bg['position'] ) . ' / ' . esc_attr( $blog_head_bg['size'] ) . ';"';
		}
